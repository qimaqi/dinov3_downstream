...
DEFAULT_TARGET_ROOT="/usr/bmicnas01/data-biwi-01/ct_video_mae_bmicscratch/data/nnUNet_raw/Dataset069_FOMO26_Meningioma_FLAIR/imagesTr_feature_gt_fomo10k"
...
for nii_path in "${INPUT_DIR}"/*.nii.gz; do
  ...
  python "${PY_SCRIPT}"     --input_nii "${nii_path}"     --output_dir "${case_output_dir}"     --slice_method gt     "$@"
done
