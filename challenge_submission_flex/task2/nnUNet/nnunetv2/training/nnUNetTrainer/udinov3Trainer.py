...
def draw_tsne_points_colored_by_instance_2D(
    output_np,
    target_np,
    save_folder,
    batch_id=0,
    ...
    min_pixels_per_instance=10,
    normalize_feature=False,
    random_state=42,
):
    print("output_np", output_np.shape, 'target_np', target_np.shape )
    # output_np (3072, 32, 32) target_np (1, 512, 512)
    """
    output_np: (C, H, W)
    target_np: (1, H, W) or (H, W)
    each non-zero value in target_np is one instance id
    """
    ...
    C, H, W = output_np.shape

    feat_flat = output_np.reshape(C, -1).T   # (N, C)
    inst_flat = inst_map.reshape(-1)         # (N,)
    ...
    for inst_id in unique_ids:
        ...
        if len(idx) < min_pixels_per_instance:
            continue
    ...
    if len(sampled_features) == 0:
        print("No valid instance points for 2D t-SNE.")
        return
    ...
    if sampled_features.shape[0] < 2:
        print("Not enough points for 2D t-SNE.")
        return
    ...
    for inst_id in uniq_inst:
        ...
        plt.text(center[0], center[1], str(int(inst_id)), fontsize=8, alpha=0.9)

    plt.title(f"2D t-SNE of pixel features colored by instance (batch {batch_id})")
    plt.xlabel("t-SNE 1")
    plt.ylabel("t-SNE 2")
    plt.tight_layout()

    save_path = join(save_folder, f"tsne_points_by_instance_2D_batch_{str(batch_id).zfill(4)}.png")
    plt.savefig(save_path, dpi=200)
    plt.close()

    print("Saved 2D t-SNE to:", save_path)



class udinov3Trainer(nnUNetTrainer):
...
class udinov3_base_primus_Trainer(udinov3Trainer):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
                 device: torch.device = torch.device('cuda')):
...
class udinov3_base_primus_Trainer_freeze(udinov3_base_primus_Trainer):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
                 device: torch.device = torch.device('cuda')):
        # Call the parent class constructor
        super().__init__(plans, configuration, fold, dataset_json, unpack_dataset, device)
...
# class udinov3_base_primus_Trainer_freeze3D(udinov3_base_primus_Trainer):
#     def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
#                  device: torch.device = torch.device('cuda')):
#         # Call the parent class constructor
#         super().__init__(plans, configuration, fold, dataset_json, unpack_dataset, device)
...
class udinov3_base_Trainer_linear_up(udinov3_base_primus_Trainer):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
                 device: torch.device = torch.device('cuda')):
        # Call the parent class constructor
        super().__init__(plans, configuration, fold, dataset_json, unpack_dataset, device)
    ...
    def build_network_architecture(patch_size: tuple, 
        ...
        from nnunetv2.training.nnUNetTrainer.dinov3.dinov3.models.multi_layer_fusion import MultiLayerAlignAndFusionBaseline1
        primus = MultiLayerAlignAndFusionBaseline1(embed_dim=768, patch_embed_size=16, num_classes=num_output_channels, 
                                   dino_encoder=model, freeze_backbone=True, feature_extraction=False, linear_prob=True)
...
class udinov3_base_Trainer_conv_up(udinov3_base_Trainer_linear_up):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
                 device: torch.device = torch.device('cuda')):
        # Call the parent class constructor
        super().__init__(plans, configuration, fold, dataset_json, unpack_dataset, device)
    ...
    def build_network_architecture(patch_size: tuple, 
        ...
        from nnunetv2.training.nnUNetTrainer.dinov3.dinov3.models.multi_layer_fusion import MultiLayerAlignAndFusionBaseline1
        primus = MultiLayerAlignAndFusionBaseline1(embed_dim=768, patch_embed_size=16, num_classes=num_output_channels, 
                                   dino_encoder=model, freeze_backbone=True, feature_extraction=False, linear_prob=False)
                                   # ,  interaction_indices=[2,5,8,11]

        return primus
    

class udinov3_base_Trainer_conv_up0(udinov3_base_Trainer_linear_up):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
                 device: torch.device = torch.device('cuda')):
        # Call the parent class constructor
        super().__init__(plans, configuration, fold, dataset_json, unpack_dataset, device)
...
class udinov3_base_Trainer_conv_up0_pretrain_1000e(udinov3_base_Trainer_conv_up0):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
                 device: torch.device = torch.device('cuda')):
        # Call the parent class constructor
        super().__init__(plans, configuration, fold, dataset_json, unpack_dataset, device)
...
class udinov3_base_Trainer_conv_up0_pretrain_v1_1000e(udinov3_base_Trainer_conv_up0):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
                 device: torch.device = torch.device('cuda')):
        # Call the parent class constructor
        super().__init__(plans, configuration, fold, dataset_json, unpack_dataset, device)
...
class udinov3_base_Trainer_linear_prob_pretrain_v1_1000e(udinov3_base_Trainer_conv_up0):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
                 device: torch.device = torch.device('cuda')):
        # Call the parent class constructor
        super().__init__(plans, configuration, fold, dataset_json, unpack_dataset, device)
...
class udinov3_base_Trainer_linear_up0(udinov3_base_primus_Trainer):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
                 device: torch.device = torch.device('cuda')):
        # Call the parent class constructor
        super().__init__(plans, configuration, fold, dataset_json, unpack_dataset, device)
...
class udinov3_base_Trainer_decoder_linear_up_version1(udinov3_base_primus_Trainer):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
                 device: torch.device = torch.device('cuda')):
        # Call the parent class constructor
        super().__init__(plans, configuration, fold, dataset_json, unpack_dataset, device)
    ...
    def build_network_architecture(patch_size: tuple, 
        ...
        from nnunetv2.training.nnUNetTrainer.dinov3.dinov3.models.multi_layer_fusion import MultiLayerAlignAndFusionBaseline1
        primus = MultiLayerAlignAndFusionBaseline1(embed_dim=768, patch_embed_size=16, num_classes=num_output_channels, 
                                   dino_encoder=model, freeze_backbone=True, feature_extraction=False, linear_prob=False, decoder_linear_prob=True)
                                   # ,  interaction_indices=[2,5,8,11]
...
class udinov3_base_Trainer3D_conv_up0(udinov3_base_Trainer_linear_up):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
                 device: torch.device = torch.device('cuda')):
        # Call the parent class constructor
        super().__init__(plans, configuration, fold, dataset_json, unpack_dataset, device)
...
########### fintuner
class udinov3_base_Linear_Prob_after_PreTrainer_2D_version0_to_rename_0(udinov3_base_primus_Trainer):
    @staticmethod
    def build_network_architecture(patch_size: tuple, 
                                   architecture_class_name: str,
                                   arch_init_kwargs: dict,
...
class udinov3_base_Linear_Prob_Multi_after_PreTrainer_2D_version0_to_reanme_0(udinov3_base_primus_Trainer):
    @staticmethod
    def build_network_architecture(patch_size: tuple, 
                                   architecture_class_name: str,
                                   arch_init_kwargs: dict,
        ...
        from nnunetv2.training.nnUNetTrainer.dinov3.dinov3.models.multi_layer_fusion import MultiLayerAlignAndFusionBaseline0
        primus = MultiLayerAlignAndFusionBaseline0(embed_dim=768, patch_embed_size=16, num_classes=num_output_channels, 
                                   dino_encoder=model, freeze_backbone=True, feature_extraction=False, linear_prob=False, freeze_adapter=True, linear_prob_multi=True)

        return primus


class udinov3_base_Linear_Prob_after_PreTrainer_2D_version0_to_rename_1(udinov3_base_Linear_Prob_after_PreTrainer_2D_version0_to_rename_0):
    ### No changes
    print("Name placeholder")



class udinov3_base_Linear_Prob_after_PreTrainer_2D_version01(udinov3_base_primus_Trainer):
    @staticmethod
    def build_network_architecture(patch_size: tuple, 
                                   architecture_class_name: str,
                                   arch_init_kwargs: dict,
        ...
        from nnunetv2.training.nnUNetTrainer.dinov3.dinov3.models.multi_layer_fusion import MultiLayerAlignAndFusionBaseline1
        primus = MultiLayerAlignAndFusionBaseline1(embed_dim=768, patch_embed_size=16, num_classes=num_output_channels, 
                                   dino_encoder=model, freeze_backbone=True, feature_extraction=False, linear_prob=True, freeze_adapter=True, decoder_prob=False, decoder_prob_multi=False)

        return primus



class udinov3_base_Decoder_Prob_after_PreTrainer_2D_version_to_rename0(udinov3_base_primus_Trainer):
    @staticmethod
    def build_network_architecture(patch_size: tuple, 
                                   architecture_class_name: str,
                                   arch_init_kwargs: dict,
        ...
        from nnunetv2.training.nnUNetTrainer.dinov3.dinov3.models.multi_layer_fusion import MultiLayerAlignAndFusionBaseline0
        primus = MultiLayerAlignAndFusionBaseline0(embed_dim=768, patch_embed_size=16, num_classes=num_output_channels, 
                                   dino_encoder=model, freeze_backbone=True, feature_extraction=False, linear_prob=False, freeze_adapter=False, decoder_prob=True, decoder_prob_multi=False)

        return primus



class udinov3_base_Decoder_Prob_after_PreTrainer_2D_version01(udinov3_base_primus_Trainer):
    @staticmethod
    def build_network_architecture(patch_size: tuple, 
                                   architecture_class_name: str,
                                   arch_init_kwargs: dict,
...
class udinov3_base_Decoder_Prob_Multi_after_PreTrainer_2D_version01(udinov3_base_primus_Trainer):
    @staticmethod
    def build_network_architecture(patch_size: tuple, 
                                   architecture_class_name: str,
                                   arch_init_kwargs: dict,
        ...
        from nnunetv2.training.nnUNetTrainer.dinov3.dinov3.models.multi_layer_fusion import MultiLayerAlignAndFusionBaseline1
        primus = MultiLayerAlignAndFusionBaseline1(embed_dim=768, patch_embed_size=16, num_classes=num_output_channels, 
                                   dino_encoder=model, freeze_backbone=True, feature_extraction=False, linear_prob=False, freeze_adapter=False, decoder_prob=False, decoder_prob_multi=True)

        return primus


class udinov3_base_Decoder_Prob_Multi_after_PreTrainer_2D_version01_scratch(udinov3_base_Decoder_Prob_Multi_after_PreTrainer_2D_version01):
    ### No changes
    print("Name placeholder")



class udinov3_base_Decoder_Prob_after_PreTrainer_2D_version_to_rename1(udinov3_base_Decoder_Prob_after_PreTrainer_2D_version_to_rename0):
    ### No changes
    print("Name placeholder")


class udinov3_base_Decoder_Prob_Multi_after_PreTrainer_2D_version_to_rename0(udinov3_base_primus_Trainer):
    @staticmethod
    def build_network_architecture(patch_size: tuple, 
                                   architecture_class_name: str,
                                   arch_init_kwargs: dict,
...
class udinov3_base_PreTrainer_2D_version0(udinov3_base_primus_Trainer):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
                 device: torch.device = torch.device('cuda')):
        # Call the parent class constructor
        super().__init__(plans, configuration, fold, dataset_json, unpack_dataset, device)
    ...
    def volume_to_pca_frames(self, data, output, target, save_folder):
        ...
        def single_batch_to_pca_frames_3D(data_np, output_np, target_np, save_folder, batch_id=0):
            ...
            if org_H*org_W*org_Z > 1000000:
                print("Too many voxels for PCA. Downsampling PCA visualization.")
                output_np = output_np[:,:,::2,::2]
                target_np = target_np[:,:,::2,::2]
                data_np = data_np[:,:,::2,::2]
                # renormalize data_np by help of self.dino_enhance 
                # "max": 6841.0,
                # "mean": 50.736724853515625,
                # "median": 59.0,
                # "min": -3024.0,
                # "percentile_00_5": -986.0,
                # "percentile_99_5": 379.0,
                # "std": 138.33982849121094
                std = self.dino_enhance.get('std', 138.33982849121094)
                mean = self.dino_enhance.get('mean', 50.736724853515625)
                data_np = (data_np - mean) / std
                # use min and max to do global normalization 
                data_np = (data_np - self.dino_enhance.get('min', -3024.0)) / (self.dino_enhance.get('max', 6841.0) - self.dino_enhance.get('min', -3024.0))
        ...
        def single_batch_to_pca_frames_2D(data_np, output_np, target_np, save_folder, batch_id=0):
            ...
            # keep full-res copies for t-SNE
            output_np_full = output_np.copy()
            target_np_full = target_np.copy()
            ...
            label_map = target_np[0]

            unique_ids = np.unique(label_map)
            id_to_color = {inst_id: i for i, inst_id in enumerate(unique_ids)}

            colored_target = np.vectorize(id_to_color.get)(label_map)

            axes[1].imshow(colored_target, cmap='nipy_spectral')

            # axes[1].imshow(target_np[0], cmap='tab20')
            ...
            plt.close(fig)
            print("Saved PCA figure to:", save_path)

            # -------- draw per-pixel t-SNE colored by instance --------
            draw_tsne_points_colored_by_instance_2D(
                output_np=output_np_full,
                target_np=target_np_full,
                save_folder=save_folder,
                batch_id=batch_id,
                max_points_per_instance=100,
                min_pixels_per_instance=10,
                normalize_feature=False,
    )
...
class udinov3_base_PreTrainer_2D_version0_with_norm(udinov3_base_PreTrainer_2D_version0):
    ...
    def on_validation_epoch_end(self, val_outputs: List[dict]):
        ...
        if self.is_ddp:
            ...
            loss_here = np.mean(outputs_collated['loss'])
            norm_loss_here = np.mean(outputs_collated['offset_reg_loss'])

        # global_dc_per_class = [i for i in [2 * i / (2 * i + j + k) for i, j, k in zip(tp, fp, fn)]]
        # mean_fg_dice = np.nanmean(global_dc_per_class)
        # self.logger.log('mean_fg_dice', mean_fg_dice, self.current_epoch)
        # self.logger.log('dice_per_class_or_region', global_dc_per_class, self.current_epoch)
        print("norm_loss_here",norm_loss_here , 'epoch', self.current_epoch)
        print("val_losses",loss_here , 'epoch', self.current_epoch)
        self.logger.log('val_losses', loss_here, self.current_epoch)


class udinov3_base_PreTrainer_2D_version01_with_norm(udinov3_base_PreTrainer_2D_version0_with_norm):
...
class udinov3_base_PreTrainer_2D_version01_with_norm_infonce(udinov3_base_PreTrainer_2D_version01_with_norm):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
                 device: torch.device = torch.device('cuda')):
        # Call the parent class constructor
        super().__init__(plans, configuration, fold, dataset_json, unpack_dataset, device)
        ...
        self.enable_deep_supervision = False
        self.supervise_rescale = 1/16
        self.diff_norm_weight=100. # strong norm loss, do no allow big change
...
class udinov3_base_PreTrainer_2D_version1_with_norm(udinov3_base_PreTrainer_2D_version0_with_norm):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
                 device: torch.device = torch.device('cuda')):
        # Call the parent class constructor
        super().__init__(plans, configuration, fold, dataset_json, unpack_dataset, device)
    ...
    def _build_loss(self):
        # debug this loss also
        loss = CosineSimilarityandL1ToMean(min_points=25)
        return loss




class udinov3_base_PreTrainer_2D_version1_with_norm_infonce(udinov3_base_PreTrainer_2D_version1_with_norm):
...
class udinov3_base_PreTrainer_2D_version1_with_norm_infonce_500e(udinov3_base_PreTrainer_2D_version1_with_norm):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
                 device: torch.device = torch.device('cuda')):
        # Call the parent class constructor
        super().__init__(plans, configuration, fold, dataset_json, unpack_dataset, device)
