...
from nnunetv2.training.nnUNetTrainer.dinov3.dinov3.eval.segmentation.models.backbone.dinov3_adapter import DINOv3_Adapter, DINOv3_Adapter_No_Dino
...
class dinoUNetTrainer(nnUNetTrainer):
    ...
    def build_network_architecture(architecture_class_name: str,
        ...
        model = DinoUNet.from_config(
            ...
            dinov3_pretrained_path='/usr/bmicnas02/data-biwi-01/fm_originalzoo/dinov3/dinov3_vitb16_pretrain_lvd1689m-73cec8be.pth', 
...
class dinoUNetTrainer_no_dino(nnUNetTrainer):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
                 device: torch.device = torch.device('cuda')):
        super().__init__(plans, configuration, fold, dataset_json, unpack_dataset, device)
    ...
    def build_network_architecture(architecture_class_name: str,
        ...
        model = DinoUNet_no_dino.from_config(
            network_config=arch_init_kwargs,
            input_channels=num_input_channels,
            num_classes=num_output_channels,
            dinov3_pretrained_path='/usr/bmicnas02/data-biwi-01/fm_originalzoo/dinov3/dinov3_vitb16_pretrain_lvd1689m-73cec8be.pth', 
        ...
        # Initialize weights for decoder
        model.decoder.apply(DinoUNet_no_dino.initialize)
        for param in model.encoder.dinov3_adapter.backbone.parameters():
            param.requires_grad = False
        return model
...
# dinoUNetTrainer
class DinoUNet_no_dino(DinoUNet):
    def _create_dinov3_encoder(self, pretrained_path, model_name, features_per_stage,
                              conv_op, norm_op, norm_op_kwargs,
                              dropout_op, dropout_op_kwargs,
                              nonlin, nonlin_kwargs, conv_bias, adapter_type="default"):
        ...
        # Create DINOv3_Adapter using correct interaction layer indices
        dinov3_adapter = DINOv3_Adapter_No_Dino(
            backbone=dinov3_backbone,
            interaction_indexes=interaction_indexes,
            pretrain_size=512,
            conv_inplane=64,
