#!/usr/bin/env bash
#SBATCH --job-name=run_dinov3_cls002_infarct_patch_cls
#SBATCH --output=sbatch_log/run_dinov3_cls002_infarct_patch_cls_%j.out
#SBATCH --nodes=1
...
OUTPUT_ROOT="${OUTPUT_ROOT:-${REPO_ROOT}/results/3d_classify/dinov3_slice_cls_each_robust_zscore_split_v0_repeat10_val4}"
...
export FOMO_CACHE_ROOT="${FOMO_CACHE_ROOT:-${PROCESSED_ROOT}/${TASK_NAME}/pretrain_fomo_10k_pretrained_dinov3_dino_base_g8_e400_p16_mri_normalize}/"
...
# python downstream/3d_classify/fomo_finetune_cls_from_slices.py   --task "${TASK_NAME}"   --processed_root "${PROCESSED_ROOT}"   --output_dir "${OUTPUT_ROOT}"   --checkpoint "${FLEXICT_2D_CHECKPOINT}"   --train_split split_80_10_10   --test_split TEST_80_10_10   --fold 0   --slice_pool patch_cls   --modality_pool each   --batch_size 1   --slice_batch_size 32   --epochs 100   --from_cache_path "${FOMO_CACHE_ROOT}"   "${EXTRA_ARGS[@]}"


# python downstream/3d_classify/fomo_finetune_cls_from_slices.py   --task "${TASK_NAME}"   --processed_root "${PROCESSED_ROOT}"   --output_dir "${OUTPUT_ROOT}"   --checkpoint "${FLEXICT_2D_CHECKPOINT}"   --train_split split_80_10_10   --test_split TEST_80_10_10   --fold 1   --slice_pool patch_cls   --modality_pool each   --batch_size 1   --slice_batch_size 32   --epochs 100   --from_cache_path "${FOMO_CACHE_ROOT}"   "${EXTRA_ARGS[@]}"
