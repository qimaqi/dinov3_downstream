#!/bin/bash
#SBATCH --job-name=train_flexcit_linear_prob_f0_custom_pretrain_mri_brats_fomo10k_48
#SBATCH --output=sbatch_log/train_flexcit_linear_prob_f0_custom_pretrain_mri_brats_fomo10k_48_%j.out
#SBATCH --nodes=1
...
export nnUNet_raw="/usr/bmicnas01/data-biwi-01/ct_video_mae_bmicscratch/data/nnUNet_raw"
export nnUNet_preprocessed="/usr/bmicnas03/data-biwi-01/qimaqi_data/data/medical_journal/meddinov3_preprocessed"
export nnUNet_results="/usr/bmicnas03/data-biwi-01/qimaqi_data/data/medical_journal/flexict_results_fomo10k/" # flexict_results_fomo10k
cd /usr/bmicnas02/data-biwi-01/qimaqi_data/workspace/medical_journal/FlexiCT_downstream/FlexiCT/downstream/segmentation
# export DATASET_ID="${DATASET_ID:-46}"
...
# export DATASET_ID="${DATASET_ID:-47}"
# export CONFIG="${CONFIG:-2d}"
# export FOLD="${FOLD:-0}"
# export TRAINER="${TRAINER:-flexict_patch8_onescale_linear_Trainer}"
# PRETRAINED_WEIGHTS="" ./run_train.sh "$@"
...
export DATASET_ID="${DATASET_ID:-48}"
export CONFIG="${CONFIG:-2d}"
export FOLD="${FOLD:-0}"
export TRAINER="${TRAINER:-flexict_patch8_onescale_linear_Trainer}"
PRETRAINED_WEIGHTS="" ./run_train.sh "$@"
...
# export DATASET_ID="${DATASET_ID:-49}"
# export CONFIG="${CONFIG:-2d}"
# export FOLD="${FOLD:-0}"
# export TRAINER="${TRAINER:-flexict_patch8_onescale_linear_Trainer}"
# PRETRAINED_WEIGHTS="" ./run_train.sh "$@"
