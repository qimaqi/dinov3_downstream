...
from flexi_ct.primus import Primus_Multiscale, Primus_Onescale, Primus_v2, _DEFAULT_SINGLE_INTERACTION_INDICE
...
def _load_flexict_teacher(backbone: nn.Module, checkpoint_path: str, num_input_channels: int) -> None:
    ...
    print(f"FlexiCT teacher missing keys: {missing}")
    print(f"FlexiCT teacher extra keys: {unexpected}")
...
def _build_flexict_lora_primus(
        patch_size,
        num_input_channels: int,
        num_output_channels: int,
        primus_cls,
...
def _build_flexict_onescale_primus(
        patch_size,
        num_input_channels: int,
        num_output_channels: int,
        runtime_patch_size: int = 8,
    ...
    return Primus_Onescale(
        in_chans=num_input_channels,
        dim=dim,
        embed_dim=model.embed_dim,
        patch_embed_size=model.patch_size,
        num_classes=num_output_channels,
        dino_encoder=model,
        interaction_indices=_DEFAULT_SINGLE_INTERACTION_INDICE,
...
class FlexiCTTrainer(nnUNetTrainer):
    ...
    def configure_optimizers(self):
        ...
        if vit_params:
            parameter_groups.append({'params': vit_params, 'lr': self.vit_lr, 'weight_decay': self.vit_weight_decay})
        optimizer = torch.optim.AdamW(parameter_groups, amsgrad=False, betas=(0.9, 0.98), fused=True)
...
class flexict_patch8_lora_Primus_v2_Trainer(flexict_patch8_Primus_v2_Trainer):
    """Runs FlexiCT Primus v2 segmentation with LoRA adapters on the FlexiCT backbone."""
    ...
    def build_network_architecture(patch_size,
                                   ...
                                   num_output_channels: int,
                                   enable_deep_supervision: bool = True) -> nn.Module:
        return _build_flexict_lora_primus(patch_size, num_input_channels, num_output_channels, Primus_v2)


class flexict_patch8_freeze_Primus_v2_Trainer(flexict_patch8_Primus_v2_Trainer):
    """Runs FlexiCT Primus v2 segmentation with a frozen FlexiCT backbone."""
...
class flexict_patch8_onescale_projector_patch_Trainer(FlexiCTTrainer):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict,
                 device: torch.device = torch.device('cuda')):
        super().__init__(plans, configuration, fold, dataset_json, device)
        self.initial_lr = 5e-5
    ...
    def build_network_architecture(patch_size,
                                   ...
                                   num_output_channels: int,
                                   enable_deep_supervision: bool = True) -> nn.Module:
        return _build_flexict_onescale_primus(
            patch_size,
            num_input_channels,
            num_output_channels,
            decoder_type="patch",
...
class flexict_patch8_onescale_linear_Trainer(FlexiCTTrainer):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict,
                 device: torch.device = torch.device('cuda')):
        super().__init__(plans, configuration, fold, dataset_json, device)
        self.initial_lr = 5e-5
